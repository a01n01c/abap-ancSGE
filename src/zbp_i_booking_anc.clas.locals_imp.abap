...
* 1) define the data buffer
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_buffer.
             INCLUDE TYPE   ztbooking_anc AS data.
...
CLASS lsc_zi_booking_001 IMPLEMENTATION.
  ...
  METHOD save.
    DATA lt_data TYPE STANDARD TABLE OF ztbooking_anc.

    lt_data = VALUE #(  FOR row IN lhc_buffer=>mt_buffer WHERE  ( flag = 'C' ) (  row-data ) ).
    IF lt_data IS NOT INITIAL.
      INSERT ztbooking_anc FROM TABLE @lt_data.
    ENDIF.
    ...
    IF lt_data IS NOT INITIAL.
      DELETE ztbooking_anc FROM TABLE @lt_data.
    ENDIF.
    ...
    IF lt_data IS NOT INITIAL.
      UPDATE ztbooking_anc FROM TABLE @lt_data.
    ENDIF.
