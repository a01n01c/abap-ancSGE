CLASS zbp_generate_bookings_anc DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zbp_generate_bookings_anc IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.
    DATA:it_bookings TYPE TABLE OF ztbooking_ANC.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ztbooking_ANC.
*   insert the new table entries
    INSERT ztbooking_ANC FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM ztbooking_ANC INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
